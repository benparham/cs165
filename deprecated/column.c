...
static int columnUpdateHeader(column *col, error *err) {
	void *serial;
	int serialBytes;

	switch (col->storageType) {
		case COL_UNSORTED:
			// unsortedSerializeHeader(col->columnHeader, &serial, &serialBytes);
			break;
		case COL_SORTED:
			// sortedSerializeHeader(col->columnHeader, &toWrite, &bytesToWrite);
			break;
		case COL_BTREE:
			// btreeSerializeHeader(col->columnHeader, &toWrite, &bytesToWrite);
			break;
		default:
			err->err = ERR_INTERNAL;
			err->message = "Unsupported column storage type";
	...
	if (fwrite(serial, serialBytes, 1, col->fp) < 1) {
		err->err = ERR_INTERNAL;
		err->message = "Unable to write header to column file";
		return 1;
	}
